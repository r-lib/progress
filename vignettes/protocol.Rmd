...
report progress or status, and the server reports it on the screen, or in
an IDE.

## Message Conditions

Clients communicate to the server via _message conditions._ The server
does not communicate to the clients.

The message conditions must have at least S3 classes `progress_message` and
`condition`. They may have additional classes as well.

A message condition is a named list, with at least an element named `type`.
If the server catches a `progress_message` condition that does not contain
a `type` member, it should re-throw it. If a condition contains a type that
is unknown to the server, it should ignore it. The server may log these
events.

A message condition contains a `version` field, which is the client's
protocol version.

## Subprocesses

The callr package automatically re-throws `progress_message` conditions
in the parent process, so progress bars work across (sub)processes.

To allow progress bars for subprocesses, progress conditions include
the process id (pid) of the process, as the `pid` member. The server may
...
types may contain. A client side API will usually not expose some of these
parameters to the end user of the progress bar. E.g. `pid` can be easily
added automatically, so it a new `id`  for `add_job()`, etc.
...
* `pid = Sys.getpid()`: integer, process id of the calling process.
    ...
    process, without the process schedule (e.g. callr) having to know
...
* `type = "loop"`: string, job type. See Section "Job Types" for the
...
* `auto-estimate = TRUE`: whether the server _should_ try to estimate
...
* `pid`: process id.
* `id`: string, job id.
* `progress`: numeric, the current completeness status of a job.
* `tokens = list()`: extra job data. See Section "Custom Job Data"
    below.

### `add_job_progress` -- add to progress status

Parameters:

* `pid`: process id.
* `id`: string, job id.
* `increment = 1L`: numeric, to number of additional progress units
    completed.
* `tokens = list()`: extra job data. See Section "Custom Job Data"
    below.
...
* `pid`: process id.
* `id`: string, job id.
* `status`: string, the new status label.
* `tokens = list()`: extra job data. See Section "Custom Job Data"
    below.
...
* `type`: type of the output: `message` or `warning` currently.
...
If a package wishes to emit progress message conditions, it can implement
a job counter for itself, and prefix the id of the job with the package
name.
...
* `loop`: a `for` loops, or an iterator, i.e. `lapply` or `purrr::map`.
...
* `current`: the number of total completed progress units.
...
```
A loop for which `total` is not known, must be displayed differently, e.g.
as
```
...
specified in the `format` element of the condition  message that creates
the job. The custom format string may refer to built in and custom tokens.
