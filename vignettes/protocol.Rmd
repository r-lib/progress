...
The server uses top level handlers (when they'll be available in R), or a
wrapper function `with_progress(expr)` to catch `progress_message`
conditions and report them.

A message condition is a named list, with at least an element named
`msgtype`. If the server catches a `progress_message` condition that does
not contain an `msgtype` member, it should re-throw it. If a condition
...
specified in the `format` element of the `add_job` condition message that
